...
class SpanModelBase(EncoderBaseModel):
    def __init__(self, config, num_entity_types=0, with_encoder=True, seq_config=None):
        ...
        self.span_hidden_size = self.seq_hidden_size * (
            2 * int(config["span_w_boundary"]) + int(config["span_w_tokens"])
        )
        if config["use_span_width"]:
            self.span_hidden_size += self.feat_hidden_size
        self.emb_span_width = self.make_embedding(
            self.max_span_width, self.feat_hidden_size
        )
        ...
        self.span_type_ffnn = self.make_ffnn(
    ...
    def debatch(
        ...
        for doc_i, num_doc_tokens in enumerate(doc_len):
            ...
            doc_tok_start_or_end, doc_sent_map = (
                tok_start_or_end[doc_i],
                sent_map[doc_i],
            )
            ...
            doc_entity_pairs_h = (
                entity_pairs_h[doc_i] if entity_pairs_h is not None else None
            )
            doc_entity_pairs_t = (
                entity_pairs_t[doc_i] if entity_pairs_t is not None else None
            )
            doc_rel_labels = rel_labels[doc_i] if rel_labels is not None else None

            doc_mention_starts = (
                mention_starts[doc_i] if mention_starts is not None else None
            )
            ...
            doc_mention_cluster_id = (
                mention_cluster_id[doc_i] if mention_cluster_id is not None else None
            )
            if doc_mention_cluster_id is not None:
                doc_mention_cluster_id += 1
                doc_mention_type += 1
            doc_flattened_rel_labels = (
                flattened_rel_labels[doc_i]
                if flattened_rel_labels is not None
                else None
            )
    ...
    def forward(
        ...
        if tokens is None:
            tokens = self.encode(
                doc_len, input_ids, attention_mask, token_type_ids, is_max_context
            )
        is_training = mention_cluster_id is not None
        doc_returns = []
        for doc in self.debatch(
            doc_len,
            tokens,
            tok_start_or_end=tok_start_or_end,
            sent_map=sent_map,
            entities=entities,
            entity_types=entity_types,
            entity_pairs_h=entity_pairs_h,
            entity_pairs_t=entity_pairs_t,
    ...
    def extract_spans(
        ...
        candidate_starts = torch.unsqueeze(
            torch.arange(0, num_tokens, device=device), 1
        ...
        candidate_end_sent_idx = sentence_indices[
            torch.min(candidate_ends, torch.tensor(num_tokens - 1, device=device))
        ]
        candidate_mask = (candidate_ends < num_tokens) & (
        ...
        candidate_mask = (
            tok_start_or_end[candidate_starts] & tok_start_or_end[candidate_ends]
        )
        candidate_starts, candidate_ends = (
            candidate_starts[candidate_mask],
            candidate_ends[candidate_mask],
        ...
        if gold_cluster_map is None:
            ...
            candidate_cluster_ids = torch.matmul(
                torch.unsqueeze(gold_cluster_map, 0).to(torch.float), same_span
            )
            candidate_cluster_ids = torch.squeeze(
                candidate_cluster_ids.to(torch.long), 0
            )
            candidate_type_labels = torch.matmul(
                torch.unsqueeze(gold_types, 0).to(torch.float), same_span
            )
            candidate_type_labels = torch.squeeze(
                candidate_type_labels.to(torch.long), 0
            )
        span_start_emb, span_end_emb = tokens[candidate_starts], tokens[candidate_ends]
        candidate_emb_list = (
            [span_start_emb, span_end_emb] if conf["span_w_boundary"] else []
        )
        if conf["span_w_tokens"]:
            candidate_tokens = torch.unsqueeze(
                torch.arange(0, num_tokens, device=device), 0
            ).repeat(num_candidates, 1)
            candidate_tokens_mask = (
                candidate_tokens >= torch.unsqueeze(candidate_starts, 1)
        ...
        if conf["use_span_type"]:
            ...
            candidate_mention_scores = torch.squeeze(
                self.span_type_ffnn(candidate_span_emb), -1
            )  # [num candidates]
        ...
        num_top_spans = int(
            min(conf["max_num_extracted_spans"], conf["top_span_ratio"] * num_tokens)
        )
        if num_top_spans == 0:
            num_top_spans = 1
        if conf["approx_pruning"]:
            ...
            selected_idx_cpu = self.get_top_spans(
                candidate_idx_sorted_by_score,
                candidate_starts_cpu,
                candidate_ends_cpu,
                num_top_spans,
        ...
        top_span_type_logits = (
            candidate_type_logits[selected_idx] if conf["use_span_type"] else None
        )
        top_span_cluster_ids = (
            candidate_cluster_ids[selected_idx]
            if gold_cluster_map is not None
            else None
        )
...
class CorefModel(SpanModelBase):
    def __init__(self, config, num_entity_types=0, with_encoder=True, seq_config=None):
        super(CorefModel, self).__init__(
            config, num_entity_types, with_encoder, seq_config
        )
        ...
        self.pair_hidden_size = self.pair_span_hidden_size * (
            2 + int(config["pair_with_similarity"])
        )
        if config["use_antecedent_distance"]:
            self.pair_hidden_size += self.feat_hidden_size
        if config["use_speaker_indicator"]:
            self.pair_hidden_size += self.feat_hidden_size
        self.emb_top_antecedent_distance = self.make_embedding(
            10, self.feat_hidden_size
        )
        ...
        self.antecedent_distance_score_ffnn = self.make_ffnn(
            self.feat_hidden_size, 0, 1
        )
        self.pair_span_anaphora_transform = self.make_ffnn(
            self.span_hidden_size, 0, self.pair_span_hidden_size
        )
        self.pair_span_antecedent_transform = self.make_ffnn(
            self.span_hidden_size, 0, self.pair_span_hidden_size
        )
        self.coarse_bilinear = self.make_ffnn(
            self.pair_span_hidden_size, 0, self.pair_span_hidden_size
        )
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        if conf["pair_span_transform"]:
            pair_anaphora_emb = torch.tanh(
                self.pair_span_anaphora_transform(top_span_emb)
            )
            pair_antecedent_emb = torch.tanh(
                self.pair_span_antecedent_transform(top_span_emb)
            )
        else:
        ...
        pairwise_mention_scores = top_span_mention_scores.unsqueeze(
            1
        ) + top_span_mention_scores.unsqueeze(0)
        span_range = torch.arange(0, num_top_spans, device=device)
        antecedent_offsets = torch.unsqueeze(span_range, 1) - torch.unsqueeze(
            span_range, 0
        )
        antecedent_mask = antecedent_offsets >= 1
        top_span_speaker_ids = (
            speaker_ids[top_span_starts] if speaker_ids is not None else None
        )
        (
            ...
            _,
        ) = self.do_antecedent_scoring(
            pairwise_mention_scores,
            pair_anaphora_emb,
            pair_antecedent_emb,
            antecedent_offsets,
            antecedent_mask,
        ...
        top_antecedent_scores = torch.cat(
            [torch.zeros(num_top_spans, 1, device=device), top_antecedent_scores],
            dim=-1,
        )
        ...
        loss, (log_marginalized, log_norm) = losses.get_mention_ranking_loss(
            top_antecedent_scores,
            top_span_cluster_ids,
            top_antecedent_idx,
            top_antecedent_mask,
        ...
        if conf["mention_loss_coef"]:
            if conf["use_span_type"]:
                ...
                (
                    loss_gold_mention,
                    loss_non_gold_mention,
                ) = losses.get_mention_score_loss(
        ...
        if self.debug:
            if self.forward_steps % (conf["report_frequency"] * 2) == 0:
                ...
                if conf["mention_loss_coef"]:
                    logger.info(
                        "gold/non-gold mention loss: %.4f/%.4f"
                        % (loss_gold_mention, loss_non_gold_mention)
                    )
                logger.info(
                    "norm/gold: %.4f/%.4f" % (log_norm.sum(), log_marginalized.sum())
                )
        self.forward_steps += 1

        return loss, outputs

    def do_antecedent_scoring(
        self,
        pairwise_mention_scores,
        pair_anaphora_emb,
        pair_antecedent_emb,
        ...
        antecedent_mask,
        max_top_antecedents,
        span_speaker_ids=None,
    ):
        conf = self.config
        pairwise_bilinear_scores = self.get_bilinear_pairwise_scores(
            pair_anaphora_emb, pair_antecedent_emb, antecedent_offsets, antecedent_mask
        )
        ...
        (
            ...
            top_antecedent_mask,
            top_antecedent_offsets,
        ) = self.coarse_antecedent_prune(
            pairwise_fast_scores,
            antecedent_mask,
            antecedent_offsets,
            max_top_antecedents,
        )
        if not conf["fine_grained"]:
            ...
            top_pairwise_slow_scores = self.get_slow_pairwise_scores(
                pair_anaphora_emb,
                pair_antecedent_emb,
                top_antecedent_idx,
                top_antecedent_offsets,
    ...
    def get_bilinear_pairwise_scores(
        self,
        pair_anaphora_emb,
        pair_antecedent_emb,
        antecedent_offsets,
        ...
        pairwise_bilinear_scores = torch.matmul(
            self.coarse_bilinear(pair_anaphora_emb),
    ...
    @classmethod
    def coarse_antecedent_prune(
        cls, pairwise_scores, antecedent_mask, antecedent_offsets, max_top_antecedents
    ):
        top_pairwise_scores, top_antecedent_idx = torch.topk(
            pairwise_scores, k=max_top_antecedents, dim=-1
        )
        top_antecedent_mask = util.batch_select_2d(antecedent_mask, top_antecedent_idx)
        top_antecedent_offsets = util.batch_select_2d(
            antecedent_offsets, top_antecedent_idx
        )
        return (
            top_pairwise_scores,
            top_antecedent_idx,
    ...
    def get_slow_pairwise_scores(
        ...
        if conf["use_antecedent_distance"]:
            ...
            antecedent_distance_emb = self.emb_top_antecedent_distance(
                antecedent_distance
            )
        ...
        anaphora_emb = torch.unsqueeze(anaphora_emb, 1).repeat(
            1, max_top_antecedents, 1
        )
    ...
    def get_predicted_clusters(
        ...
        def is_singleton(idx):
            return (
                ...
                else (span_mention_scores[idx] > 0)
            )

        predicted_antecedents = cls.get_predicted_antecedents(
        ...
        for i, predicted_idx in enumerate(predicted_antecedents):
            ...
            if antecedent_cluster_id == -1:
                ...
                predicted_types.append(
                    [span_types[predicted_idx] if span_types else None]
                )
            ...
            predicted_types[antecedent_cluster_id].append(
                span_types[i] if span_types else None
            )
    ...
    @staticmethod
    def get_predicted_pseudo_clusters(
        span_starts,
        span_ends,
        span_mention_scores,
        span_types,
        ...
        for span_i in range(len(span_starts)):
            mention = span_starts[span_i], span_ends[span_i]
            mention_tok = subtoken_map[mention[0]], subtoken_map[mention[1]]
            if is_singleton(span_i) > 0:
                predicted_clusters.append([mention_tok])
                predicted_clusters_subtok.append([mention])
                predicted_clusters_idx.append([span_i])
                predicted_types.append([span_types[span_i] if span_types else None])
                mentions.add(mention)
            continue
        mentions = sorted(list(mentions))
        mention_starts = [s for s, e in mentions]
