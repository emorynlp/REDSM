...
class BaseRunner(ABC):
    def __init__(self, config_name, gpu_id=None, seed=None, save_log=True):
        ...
        # self.device = torch.device("cpu" if gpu_id is None else "cuda")
    ...
    @classmethod
    def get_scheduler(
        cls, optimizer, total_update_steps, warmup_ratio=None, scheduler_name="linear"
    ):
        ...
        if scheduler_name == "custom":
            ...
            scheduler = get_scheduler(
                scheduler_name, optimizer, warmup_steps, total_update_steps
            )
            return scheduler
    ...
    def load_model_checkpoint(self, model, init_suffix, init_config=None, strict=False):
        load_dir = (
            self.config["log_dir"]
            if init_config is None
            else join(self.config["log_root"], init_config)
        )
        path_ckpt = join(load_dir, f"model_{init_suffix}.bin")
        model.load_state_dict(
            torch.load(path_ckpt, map_location=torch.device("cpu")), strict=strict
        )
